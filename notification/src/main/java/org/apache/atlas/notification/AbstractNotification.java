...
public abstract class AbstractNotification implements NotificationInterface {
    ...
    public <T> void send(NotificationType type, List<T> messages) throws NotificationException {
        send(type, messages, new HashMap<String, String>());
    }

    @Override
    public <T> void send(NotificationType type, List<T> messages, Map<String, String> sqlInfo) throws NotificationException {
        List<String> strMessages = new ArrayList<>(messages.size());

        if (type == NotificationType.MONITOR) {
            for (int index = 0; index < messages.size(); index++) {
                createMontitorMessages(messages.get(index), strMessages, sqlInfo);
            }
        } else {
            for (int index = 0; index < messages.size(); index++) {
    ...
    /**
     * Get the monitor message JSON from the given object.
     *
     * @param message  the message in object form
     *
     * @return the message as a JSON string
     */
    public static void createMontitorMessages(Object message, List<String> msgJsonList, Map<String, String> sqlInfo) {
        ...
        KafkaMessageJson kafkaMessageJson = new KafkaMessageJson(msgJson, message, getHostAddress(), getCurrentUser(), sqlInfo);  //增加更多信息，合并到下面

        if (!msgLengthExceedsLimit) {
            msgJsonList.add(kafkaMessageJson.toString());
        }
    }
